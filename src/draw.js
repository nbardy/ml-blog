...
export function drawField(canvas, field, config) {
  ...
  for (var y = 0; y < totaly; y++) {
    ...
    for (var x = 0; x < totalx; x++) {
      ...
      (xmag = field.get(i, 0)), (ymag = field.get(i, 1));
...
export function drawScene(canvas, particles, field, config) {
  const ctx = canvas.getContext("2d");
  const lastFrameImgData = canvas.toDataURL();

  // Draw tinted rect over to partially clear
  const [r, g, b] = config.backgroundColor;
  ctx.fillStyle = `rgb(${r},${g},${b}, ${config.alphaBlend})`;
  ...
  // Draw a transparent version of the last frame
  if (config.trailRate) {
    var img = new Image();
    const opacity = config.trailRate;
    img.src = lastFrameImgData;

    img.opacity = opacity / 100;
    img.MozOpacity = opacity / 100;
    img.KhtmlOpacity = opacity / 100;
    img.filter = "alpha(opacity=" + opacity + ")";
    // ctx.globalAlpha = config.trailRate;
